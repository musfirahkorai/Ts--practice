...
(function (Genre) {
    ...
    Genre[Genre["Fantasy"] = 2] = "Fantasy";
    Genre[Genre["Biography"] = 3] = "Biography";
...
// Functions
function addAuthor(library, author) {
    library.authors.push(author);
}
...
function getBooksByAuthor(library, authorId) {
    return library.books.filter(book => book.authorId === authorId);
}
function findAuthorById(library, authorId) {
    return library.authors.find(author => author.id === authorId);
}
function findBookById(library, bookId) {
    return library.books.find(book => book.id === bookId);
}
function listAuthors(library) {
    return library.authors;
}
...
console.log(listBooks(library));
// Additional functionality
function updateBook(library, bookId, updatedInfo) {
    const book = findBookById(library, bookId);
    if (book) {
        Object.assign(book, updatedInfo);
    }
}
function updateAuthor(library, authorId, updatedInfo) {
    const author = findAuthorById(library, authorId);
    if (author) {
        Object.assign(author, updatedInfo);
    }

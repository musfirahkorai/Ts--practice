...
// Functions
function addTask(manager, task) {
    manager.tasks.push(task);
}
function addUser(manager, user) {
    manager.users.push(user);
}
function getTasksByStatus(manager, status) {
    return manager.tasks.filter(task => task.status === status);
}
function getTasksByPriority(manager, priority) {
    return manager.tasks.filter(task => task.priority === priority);
}
function findTaskById(manager, taskId) {
    return manager.tasks.find(task => task.id === taskId);
}
function findUserById(manager, userId) {
    return manager.users.find(user => user.id === userId);
}
function assignTaskToUser(manager, taskId, userId) {
...
function completeTask(manager, taskId) {
    const task = findTaskById(manager, taskId);
    if (task) {
...
function listAllTasks(manager) {
    return manager.tasks;
}
function listAllUsers(manager) {
